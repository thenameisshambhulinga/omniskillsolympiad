  ...
  const cards = [
    {
      ...
      value: status?.stage?.label?.toUpperCase?.() ?? "PENDING",
      helper: status?.stage?.description ?? "Not Available",
      icon: Layers3,
      tone: "border-cyan-400/20 bg-cyan-400/10 text-cyan-200",
    },
    {
      label: "Current OMNI Step",
      value: status?.omniStep?.label ?? "Pending",
      helper: status?.omniStep?.description ?? "Not Available",
      icon: Route,
      tone: "border-purple-400/20 bg-purple-400/10 text-purple-200",
    },
    {
      label: "Assessment Status",
      value: status?.assessmentStatus ?? "Not Started",
      helper: "Offline assessment progress",
      icon: ClipboardCheck,
      tone: getAssessmentTone(status?.assessmentStatus ?? "Not Started"),
    },
    {
      label: "Verification Status",
      value: status?.verificationStatus ?? "Pending",
      helper: "Mentor / judge review status",
      icon: BadgeCheck,
      tone: getVerificationTone(status?.verificationStatus ?? "Pending"),
    },
    {
      label: "National Rank",
      value: status?.nationalRank ?? "Pending",
    ...
    },
    {
      ...
      value: status?.engineeringTier ?? "Pending",
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="rounded-[2rem] border border-emerald-400/25 bg-emerald-400/10 px-6 py-5">
            ...
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${status?.readinessPercent ?? 0}%` }}
